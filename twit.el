...
;;          - protocol is now customizable between http, and https. (JA)
;; - 0.3.6  - updated 0.3.5 changelog
;;          - fixed problem with url compressor (thanks Wom Mendoza)
;;          - Added wordwrapping when long-lines is available and images
;;            are turned off.
...
The value returned is the current buffer."
  `(with-current-buffer (get-buffer-create ,buffer-name)
     ...
     (goto-char (point-min))
	 (when (and (not twit-show-user-images)
				(featurep 'longlines))
		   (longlines-mode 1))
     (current-buffer)))
...
(define-derived-mode twit-post-mode text-mode
  ...
  (define-key 'twit-post-mode-map [(Control c) (Control c)] 'twit-post-buffer))

;;* post query-buffer post-interface
(defun twit-buffer-up-for-post (prompt-heading initial-input url &optional reply-id)
  "Query for a post inside of a regular buffer instead of the minibuffer."
  (pop-to-buffer (get-buffer-create "*twit-post*"))
  (twit-post-mode)
  (insert (propertize initial-input
					  'twit-url-destination url
					  'twit-id reply-id))
  nil) ; signal to the caller that we are not ready for the post.
...
become deprecated."
  ...
  (let* ((reply-to (when prefix
                     (twit-get-text-property 'twit-user)))
         (post (apply twit-posting-function
                (if reply-to
...
;;* post interactive buffer query-buffer
;;;###autoload
...
Note that this retrieves the destination for posting, and reply id from
text properties if available, otherwise, assumes no reply id, and standard
status update."
  (interactive)
  (let ((url (if (twit-get-text-property 'twt-url-destination)
				 (twit-get-text-property 'twt-url-destination)
				 twit-update-url))
		(parent-id (twit-get-text-property 'twit-id))
		(post (buffer-substring-no-properties (point-min) (point-max))))
    (if (> (length post) twit-max-tweet)
        (error twit-too-long-msg)
        (twit-post-status url post parent-id))))
...
;;* posting
(when nil
	  (let* ((summary " - 0.3.5 - Multiple bugfixes, Friendship graphing API added, Twanalist integraion, Diarrhea filter, infrastructure for i18n and popup buffers/")
         (short-summary "bufixes, Twanalist integraion, Diarrhea filter.  i on twit.el to auto-install!")
         (twit-post (format "New #twit.el: V %s %s" twit-version-number short-summary)))
		(when (> (length twit-post) 140)
			  (error "Post is gonna be to long.  Not updating yet."))
