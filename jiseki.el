...
;;* custom face status
(defface jiseki-item-running
  '((default
	  :foreground "ForestGreen"
	  :inherit 'jiseki-item-face))
  "A currently running or active item"
  :group 'jiseki)

;;* custom face status
(defface jiseki-item-paused
  '((default
	  :foreground "gold2"
	  :inherit 'jiseki-item-face))
  "A paused item"
...
;;* custom face status
(defface jiseki-item-stopped
  '((default
	  :foreground "IndianRed"
	  :inherit 'jiseki-item-face))
  "An item that is 'stopped'  (i.e. an exited process)"
  :group 'jiseki)

;;* custom face status
(defface jiseki-item-error
  '((default
	  :foreground "blue3"
	  :inherit 'jiseki-item-face))
  "An item that is in an error state"
  :group 'jiseki)
...
(setq processes-source
	  '((name . "Show All Open Processes")
		...
		(display . (lambda () (apply 'concat " Proc: "
									 (mapcar (lambda (p) (concat " "
																 (propertize (process-name p)
																			 'face
																			 (jiseki-get-face-from-status (process-status p)))
																 " "))
 											 (process-list)))))))

;;* source process helper
(defun jiseki-get-face-from-status (stat)
  "Return a face from a status symbol."
  (case stat
	((run) "jiseki-item-running")
	((stop) "jiseki-item-stopped")
	((exit) "jiseki-item-stopped")
	((signal) "jiseki-item-error")
	((open) "jiseki-item-running")
	((listen) "jiseki-item-paused")
	((closed) "jiseki-item-stopped")
	((connect) "jiseki-item-running")
	((failed) "jiseki-item-error")
	(t "jiseki-item-error")))
