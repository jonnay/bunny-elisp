...
;;
;;  `todochiku-in'
;;    Send a todochiku MESSAGE at MINS.
;;  `todochiku-test'
;;    Test out todochiku and make sure it works.
;;
...
;;    default = (case system-type (windows-nt "C:/Program Files/full phat/Snarl/extras/sncmd/snarl_command.exe") (darwin "/usr/local/bin/growlnotify") (t "notify-send"))
;;  `todochiku-command-args'
;;    A function to return a list of arguments for the notification command.
;;    default = (case system-type (windows-nt (quote todochiku-snarl-args)) (darwin (quote todochiku-growlnotify-args)) (t (quote todochiku-notify-args)))
;;  `todochiku-message-too'
;;    Display todochiku as messages as well as notifications.
;;    default = nil
...
;;    Whether or not pings from apt-disp-window should show up in Emacs and growl.
;;    default = (quote t)
;;  `todochiku-debug'
;;    Variable for debugging ToDoChiKu.
;;    default = nil
...
;; - Build better backend support, so that other command lines can be used
;; - Build JSON backend
;; - Add this backend: mumbles-send "test title" "test message"

;;; CHANGELOG:
;; V0.1    - Complete re-write of the backend.
;;         - fix path to snarl commmand (extras vs exrras)
;;         - debugging into a buffer for commands
;;         - better backend support
;;         - added interactive test function
;; V0.0.7  - Added YaOddMuse interface (mislabeled as V0.7)
...
(defcustom todochiku-command
  (case system-type
    (windows-nt "C:/Program Files/full phat/Snarl/extras/sncmd/snarl_command.exe")
...
(defcustom todochiku-command-args
  (case system-type
	(windows-nt 'todochiku-snarl-args)
	(darwin 'todochiku-growlnotify-args)
	(t 'todochiku-notify-args))
  "A function to return a list of arguments for the notification command.

The possible values are:
'todochiku-snarl-command
'todochiku-growlnotify-command
'todochiku-notify-command
'todochiku-mumbles-command"
  :type 'function
  :group 'todochiku)

(defcustom todochiku-message-too nil
  "Display todochiku as messages as well as notifications.
...
(defcustom todochiku-tooltip-too nil
  "Display todochiku as a tooltip.
...
(defcustom todochiku-display-appts-in-window-too 't
  "Whether or not pings from apt-disp-window should show up in Emacs and growl.
This is really only useful if you use the appt package (i.e. from org mode)."
  :type '(boolean)
  :group 'todochiku)
...
See the variable `todochiku-icons' for a list of available icons."
  ...
  (when (not (string= todochiku-command ""))
		(apply 'start-process
			   "todochiku"
			   (if todochiku-debug "*todochiku debug*" nil)
			   todochiku-command
...
(defun todochiku-get-arguments (title message icon)
  ...
  (let ((args (or todochiku-command-args
				  (case system-type
					('windows-nt 'todochiku-snarl-args)
					('darwin 'todochiku-growlnotify-args)
					(t 'todochiku-notify-args))))))
  (apply todochiku-command-args (list title message icon)))

;;* backend snarl
(defun todochiku-snarl-args (title message icon)
  "Backend for snarl command"
  (list "/M" title message icon))

;;* backend growl
(defun todochiku-growlnotify-args (title message icon)
  "Backend for growl-notify"
  (list title "-m" message "--image" icon ))

;;* backend notify JasonMcBrayer
(defun todochiku-notify-args (title message icon)
  "Backend for libnotify"
  (list "-i" icon "-t" (int-to-string (* 1000 todochiku-timeout)) title message))
...
(defun todochiku-in (message mins)
  "Send a todochiku MESSAGE at MINS.
Can take a prefix arg for the number of mins to wait."
...
(defun growl (title message)
  "Alias for `todochiku-message'.  Send TITLE and MESSAGE to `todochiku-message'."
